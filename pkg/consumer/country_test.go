...
func TestCountryConsumerList(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		countries, _ := GetCountryConsumer().List()
		assert.Equal(t, tc.ExpectedCountries, countries)
	}
}

func TestCountryConsumerInfo(t *testing.T) {
	testCases := []struct {
		Name                string
		NewRequestFunction  func(method string, url string, body io.Reader) (*http.Request, error)
	...
	for _, tc := range testCases {
		...
		countryInfo, _ := GetCountryConsumer().Info("BR")
