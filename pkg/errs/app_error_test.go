...
func TestAppErrorAsInterface(t *testing.T) {
	defer func() {
		if r := recover(); r != nil {
			err := r.(error)
			assert.EqualError(t, err, "CMN010: error converting string to int")
		} else {
			assert.NotNil(t, r)
		}
	}()

	panic(ErrConvertingStringToInt)
}

func TestAppErrorCode(t *testing.T) {
	log := applog.Log
	err := ErrConvertingStringToInt.Throw(log)
	te := err.(*appError)
	var correct bool
	switch te.Code() {
	case ErrConvertingStringToInt.Code():
		correct = true
	default:
		correct = false
	}
...
func TestIs(t *testing.T) {
	log := applog.Log
	err := ErrConvertingStringToInt.Throw(log)
	res := err.Is(ErrConvertingStringToInt)
	assert.True(t, res)
}
